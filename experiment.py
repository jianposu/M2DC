...
def step(model, criterion, dyn_v, dyn_a, sampling_endpoints, t, label, reg_lambda, clip_grad=0.0, device='cpu', optimizer=None, scheduler=None, tau=0.5):
    if optimizer is None: model.eval()
    else: model.train()

    # run model
    logit, attention, latent, _ = model(dyn_v.to(device), dyn_a.to(device), t.to(device), sampling_endpoints)
    if optimizer is None:
        loss = criterion(logit, label.to(device))
    else:
        loss = criterion(logit/tau, label.to(device))
...
def train(argv):
    ...
    elif argv.dataset=='HCPrest512': dataset = DatasetHCPRest512(argv.sourcedir, roi=argv.roi, k_fold=argv.k_fold, smoothing_fwhm=argv.fwhm)
    elif argv.dataset=='RestMDD': dataset = DatasetRestMDD(argv.sourcedir, argv, roi=argv.roi, k_fold=argv.k_fold, smoothing_fwhm=argv.fwhm)
    else: raise
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=argv.minibatch_size, shuffle=True, num_workers=4, pin_memory=True)
    #dataloader = torch.utils.data.DataLoader(dataset, batch_size=argv.minibatch_size, drop_last=False, shuffle=False, num_workers=4, pin_memory=True)
        ...
        for epoch in range(checkpoint['epoch'], argv.num_epochs):
            ...
            for i, x in enumerate(tqdm(dataloader, ncols=60, desc=f'k:{k} e:{epoch}')):
                ...
                logit, loss, attention, latent = step(
                    ...
                    tau=argv.tau)
                pred = logit.argmax(1)
                prob = torch.nan_to_num(logit.softmax(1))

                if torch.any(torch.isnan(prob)):
                    # print('logit ',logit.detach().cpu().numpy())
                    a = torch.isnan(dyn_a)
                    b = a==True
                    # print('dyn_a_nan ',b.nonzero())
...
def test(argv):
    ...
    device = torch.device(argv.device) if torch.cuda.is_available() else torch.device("cpu")
    ...
    for k in range(argv.k_fold):
        ...
        if argv.dataset=='rest' or argv.dataset=='HCPrest512' or argv.dataset=='RestMDD':
