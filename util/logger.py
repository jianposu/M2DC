...
class LoggerSTAGIN(object):
    ...
    def evaluate(self, k=None, initialize=False, option='mean'):
        ...
        if not self.k_fold is None and k is None:
            ...
            sensitivity = []
            for k in range(self.k_fold):
                tn, fp, fn, tp = metrics.confusion_matrix(samples['true'][k], samples['pred'][k]).ravel()
        ...
        return dict(accuracy=accuracy, precision=precision, recall=recall, specificity=specificity, sensitivity=sensitivity, roc_auc=roc_auc, f1=f1)


    def to_csv(self, targetdir, k=None, initialize=False):
        metric_dict = self.evaluate(k, initialize)
        #filename = current_site+'_metric.csv'
        filename = 'metric.csv'
        append = os.path.isfile(os.path.join(targetdir,filename))
        with open(os.path.join(targetdir, filename), 'a', newline='') as f:
            writer = csv.writer(f)
            if not append:
